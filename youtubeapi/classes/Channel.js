...
class Channel extends ChannelCompact_1.default {
	 ...
	 */
	load(data) {
		const { channelId, title, avatar, subscriberCountText } =
		...
		this.videos = [];
		this.playlists = [];
		const { tvBanner, mobileBanner, banner } =
			data.header.c4TabbedHeaderRenderer;
		this.banner =
			banner?.thumbnails && new Thumbnails_1.default().load(banner.thumbnails);
		this.tvBanner =
			tvBanner?.thumbnails &&
			new Thumbnails_1.default().load(tvBanner.thumbnails);
		this.mobileBanner =
			mobileBanner?.thumbnails &&
			new Thumbnails_1.default().load(mobileBanner.thumbnails);
		// shelves
		const rawShelves =
		...
		for (const rawShelf of rawShelves) {
			const shelfRenderer =
				rawShelf.itemSectionRenderer.contents[0].shelfRenderer;
			...
			const items = content.horizontalListRenderer?.items
				?.map((i) => {
					if (i?.gridVideoRenderer)
						return new VideoCompact_1.default({ client: this.client }).load(
							i.gridVideoRenderer
						);
					if (i?.gridPlaylistRenderer)
						return new PlaylistCompact_1.default({ client: this.client }).load(
							i.gridPlaylistRenderer
						);
					if (i?.gridChannelRenderer)
						return new ChannelCompact_1.default({ client: this.client }).load(
