...
class BaseVideo extends _1.Base {
	...
	load(data) {
		...
		this.thumbnails = new _1.Thumbnails().load(
		...
		const topLevelButtons =
			videoInfo.videoActions.menuRenderer.topLevelButtons;
		this.likeCount = common_1.stripToInt(
			BaseVideo.parseButtonRenderer(topLevelButtons[0])
		);
		this.dislikeCount = common_1.stripToInt(
			BaseVideo.parseButtonRenderer(topLevelButtons[1])
		);
		// Tags and description
		this.tags =
			((_b =
				(_a = videoInfo.superTitleLink) === null || _a === void 0
					? void 0
					: _a.runs) === null || _b === void 0
				? void 0
				: _b.map((r) => r.text.trim()).filter((t) => t)) || [];
		this.description =
			((_c = videoInfo.description) === null || _c === void 0
				? void 0
				: _c.runs.map((d) => d.text).join("")) || "";
		...
		if (secondaryContents) {
			const upNext =
				((_d = secondaryContents.find(
					(s) => "compactAutoplayRenderer" in s
				)) === null || _d === void 0
					? void 0
					: _d.compactAutoplayRenderer.contents[0]) || null;
			this.upNext = upNext
				? BaseVideo.parseCompactRenderer(upNext, this.client)
				: upNext;
			this.related.push(
				...BaseVideo.parseRelated(secondaryContents, this.client)
			);
			// Related continuation
			this.relatedContinuation =
				common_1.getContinuationFromItems(secondaryContents);
		} else {
			this.upNext = null;
			this.related = [];
		}
	...
	nextRelated(count = 1) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			for (let i = 0; i < count || count == 0; i++) {
				...
				const response = yield this.client.http.post(
					`${constants_1.I_END_POINT}/next`,
					{
	...
	static parseCompactRenderer(data, client) {
		if ("compactVideoRenderer" in data) {
			return new _1.VideoCompact({ client }).load(
				data.compactVideoRenderer
			);
		} else if ("compactRadioRenderer" in data) {
			return new _1.PlaylistCompact({ client }).load(
	...
	static parseButtonRenderer(data) {
		...
		const accessibilityData = (
			((_a = buttonRenderer.defaultText) === null || _a === void 0
				? void 0
				: _a.accessibility) || buttonRenderer.accessibilityData
		).accessibilityData;
